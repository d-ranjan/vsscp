        ...
        <div class="p-6 text-gray-900">
            ...
            @if (session('status') === 'profile-updated')
                <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 2000)"
                    ...
                    {{ __('Teacher Updated Successfully') }}</p>
            @endif

            @if (session('status') === 'profile-deleted')
                <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 2000)"
                    class="mb-4 py-2 bg-red-500 rounded-sm text-sm text-center text-gray-600 dark:text-white">
                    {{ __('Teacher Deleted Successfully') }}</p>
            @endif
                            ...
                            <td class="px-1 py-1">
                                <div class="flex items-center justify-center">
                                    <a href="{{ route('teacher.edit', $teacher->id) }}"
                                        class="mx-1 bg-green-600 p-2 border-b rounded-sm" title="Edit Teacher Profile">
                                    ...
                                    <a href="{{ route('teacher.delete', $teacher->id) }}"
                                        class="mx-1 bg-red-600 p-2 border-b rounded-sm" title="Remove Teacher Profile">
                                        <svg class="h-4 w-4 fill-current text-gray-100" aria-hidden="true"
